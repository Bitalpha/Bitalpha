...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("bitalpha"))
        return false;
...
{
    // Convert bitalpha:// to bitalpha:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bitalpha://"))
    {
        uri.replace(0, 12, "bitalpha:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BitAlpha.lnk";
}
...
{
    return GetAutostartDir() / "bitalpha.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BitAlpha-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bitalpha-qt [" + tr("command-line options") + "]                     " + "\n";
